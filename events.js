...
//  📑 Шпаргалка по событиям DOM

// ## 🖱 События мыши
// - `click` — клик (mousedown + mouseup на одном элементе).  
//   ➝ Кнопки, ссылки, интерактивные элементы.  
// - `dblclick` — двойной клик.  
//   ➝ Редактирование текста, особые действия.  
// - `mousedown` / `mouseup` — нажатие/отпускание кнопки мыши.  
//   ➝ Drag&Drop, рисование.  
// - `mousemove` — движение мыши.  
//   ➝ Подсветка, рисование, трекинг.  
// - `mouseenter` / `mouseleave` — вход/выход курсора (не всплывают).  
//   ➝ Подсветка меню, hover‑эффекты.  
// - `mouseover` / `mouseout` — наведение/уход (всплывают).  
//   ➝ Делегирование подсветки.  
// - `contextmenu` — правый клик.  
//   ➝ Кастомное контекстное меню.  
// - `wheel` — прокрутка колёсиком.  
//   ➝ Зум, кастомный скролл.

// ---

// ## ⌨️ События клавиатуры
// - `keydown` — клавиша нажата (срабатывает при удержании).  
//   ➝ Горячие клавиши, игры.  
// - `keyup` — клавиша отпущена.  
//   ➝ Завершение ввода, комбинации.  
// - `keypress` (устар.) — ввод символа.  
//   ➝ Лучше использовать `keydown` + проверку `event.key`.

// ---

// ## 📱 Pointer / Touch (сенсорные устройства)
// - `pointerdown` / `pointerup` / `pointermove` — универсальные события для мыши, пера, тач.  
//   ➝ Современный стандарт вместо `mouse*` и `touch*`.  
// - `touchstart` / `touchend` / `touchmove` — старый API для тачей.  
//   ➝ Жесты на мобильных.
...
//   ➝ Анимации интерфейса.  
// - `animationstart` / `animationend` / `animationiteration` — события CSS‑анимаций.  
//   ➝ Сложные UI‑эффекты.

// ---

// ## 📋 Clipboard (буфер обмена)
// - `copy` / `cut` / `paste` — работа с буфером.  
//   ➝ Кастомные редакторы, защита контента.
...
//   ➝ SPA‑роутинг, ленивые загрузки.

// ---

// ## 📦 Drag & Drop
// - `dragstart` / `dragend` — начало/конец перетаскивания.  
// - `dragenter` / `dragleave` — вход/выход в зону.  
// - `dragover` — элемент над зоной.  
// - `drop` — сброс.  
//   ➝ Перетаскивание файлов, карточек.

// ---

// ## 🖼 Fullscreen API
// - `fullscreenchange` — вход/выход из полноэкранного режима.  
// - `fullscreenerror` — ошибка перехода.  
//   ➝ Видео, презентации.

// ---

// ## 🈳 Composition (IME, языки)
// - `compositionstart` / `compositionupdate` / `compositionend`  
//   ➝ Ввод иероглифов, сложных символов.
